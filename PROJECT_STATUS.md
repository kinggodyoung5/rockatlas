...
- **다음 배치: O부터.**
- **상세 페이지 표시 일관성 수정(2026-07-21, 커밋 `8a03648`)**: 태그·세부장르·분위기·음반 목록이 `.join(' · ')` 한 줄 문자열이라 바로 옆 "장르 교차점"(항목별로 나뉘어 표시)과 표시 방식이 달랐던 것을 통일(전부 항목별 줄바꿈). "장르 교차점" 시대별 타임라인은 장르가 안 바뀌면 매 시대(연대)마다 같은 걸 반복 표시하던 것을 `groupEraTags()`로 연속 구간을 하나로 합쳐 "1970년대~" 식으로 표시하도록 수정(밴드별 원본 데이터에 시대 항목이 몇 개 있든 동일하게 동작). Dream Theater·Deep Purple·Kodaline의 태그·시대별 세부장르에 남아있던 영어 표기(Gemini 입고 잔재)도 한글로 통일.
- `npm run validate:data` 결과: **오류 0건** (49개 밴드 전체 통과)
- 로컬 미커밋 변경 없음 (전부 커밋·푸시·main 배포 완료)
