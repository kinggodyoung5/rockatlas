...
- **`reviewed`(검수됨)와 `published`(공개)는 현재 공개 사이트에서 완전히 동일하게 취급됨** — `publicBands = bands.filter(band => band.reviewStatus !== 'draft')`라 둘 다 무조건 공개 노출. 사용자에게 이 사실을 알리고 의도된 동작인지 확인 필요(18번 항목).
...
3. **먼저 사용자에게 확인**: `reviewed`/`published` 노출 차이를 실제로 다르게 만들지(예: `published`만 공개 목록에 노출, `reviewed`는 Studio 미리보기에서만 보이게) 여부
4. 다음 알파벳 배치(O부터, 51개 밴드 기준)를 이어서 진행 — 멤버 활동연도, 대표곡 감상 안내, 이미지 URL 실제 접속 확인, status·role 언어 오기재 여부, 태그-세부장르 정합성 확인
5. **데이터만 넣고 끝내지 말 것 — 실제 브라우저에서 상세 페이지를 열어 화면에 반영되는지 반드시 확인.**
