...
function ExplorerApp() {
  ...
  const manifesto = <section className={`manifesto manifesto-bg-${siteContent.theme.manifestoBackgroundMode}`} key="manifesto">{siteContent.theme.manifestoBackgroundMode === 'image' && siteContent.theme.manifestoImageUrl && <img className="manifesto-bg-image" src={siteContent.theme.manifestoImageUrl} alt="" style={{ objectPosition: siteContent.theme.manifestoImagePosition }} />}{siteContent.theme.manifestoBackgroundMode === 'image' && <div className="manifesto-bg-overlay" style={{ opacity: siteContent.theme.manifestoOverlayOpacity }} />}<div className="shell manifesto-inner"><span>{siteContent.manifestoLabel}</span><h2>{siteContent.manifestoTitle.split('\n').map((line, index) => <span key={index}>{index > 0 && <br />}{line}</span>)}</h2><button onClick={surpriseMe}>{siteContent.manifestoButtonLabel} <ArrowRight /></button></div></section>
  const homeMain = (
